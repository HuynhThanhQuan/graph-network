...
class KatalonGraph:
    ...
    def rescale_node_size(node_sizes):
        ...
        return node_sizes / _range * KatalonGraph.NODE_MAX_SIZE + KatalonGraph.NODE_MIN_SIZE
    ...
    def rescale_edge_with(edge_widths):
        ...
        _range = max_value - min_value
        return edge_widths / _range * KatalonGraph.EDGE_MAX_WIDTH + KatalonGraph.EDGE_MIN_WIDTH
    ...
    def init_node_traces_3d(digraph, position, colorscale='YlOrRd'):
        ...
        node_trace = go.Scatter3d(x=node_x, y=node_y, z=node_z, name='All Nodes', mode='markers', hoverinfo='text',
                                  marker=dict(showscale=True, colorscale=colorscale, reversescale=True,
                                              size=KatalonGraph.rescale_node_size(node_size),
        ...
        for i, adjacencies in enumerate(digraph.adjacency()):
            ...
            node_text.append(KatalonGraph.get_text_node_info(digraph, adjacencies[0]))
        ...
        for path in paths:
            ...
            for idx, node in enumerate(path):
                digraph.nodes[node]['frequency'] = digraph.nodes[node].get('frequency', 0) + \
                                                   KatalonGraph.NODE_INCREMENTAL_VALUE
            ...
            if self.flow_direction == KatalonGraph.DIRECTION_BOTTOM_UP:
                digraph.add_edges_from([(path[i], path[i - 1]) for i in range(len(path) - 1, 0, -1)])
                for i in range(len(path) - 1, 0, -1):
                    digraph.edges[path[i], path[i - 1]]['weight'] = \
                        digraph.edges[path[i], path[i - 1]].get('weight', 0) + KatalonGraph.NODE_INCREMENTAL_VALUE
            elif self.flow_direction == KatalonGraph.DIRECTION_TOP_DOWN:
        ...
        for edge in digraph.edges():
            ...
            go_edge = go.Scatter3d(x=[x0, x1], y=[y0, y1], z=[z0, z1],
                                   line=dict(width=min(digraph.edges[edge[0], edge[1]]['weight'] *
                                                       KatalonGraph.EDGE_SCALED_WIDTH_FACTOR,
                                                       KatalonGraph.EDGE_MAX_WIDTH),
                                             color='#888'),
                                   showlegend=False,
    ...
    def init_edge_traces_2d(digraph, position):
        ...
        for edge in digraph.edges():
            ...
            go_edge = go.Scatter(x=[x0, x1], y=[y0, y1],
                                 line=dict(width=min(digraph.edges[edge[0], edge[1]]['weight'] *
                                                     KatalonGraph.EDGE_SCALED_WIDTH_FACTOR,
                                                     KatalonGraph.EDGE_MAX_WIDTH), color='#888'),
                                 showlegend=False,
                                 text='Connections\t{}'.format(digraph.edges[edge[0], edge[1]]['weight']),
                                 mode='lines')
            go_edges.append(go_edge)
    ...
    def update_position_layout(layout, digraph, position):
        if layout == KatalonGraph.VOLCANO_LAYOUT_3D:
            return Layout.get_volcano_layout(digraph, position)
        elif layout == KatalonGraph.CYLINDER_LAYOUT_3D:
            return Layout.get_cylinder_layout(digraph, position)
        elif layout == KatalonGraph.TREE_LAYOUT_3D:
            return Layout.get_tree_layout(digraph, position)
        elif layout == KatalonGraph.TREE_CENTRALIZED_LAYOUT_3D:
            return Layout.get_tree_centralized_layout(digraph, position)
        elif layout == KatalonGraph.WORD2VECTOR_LAYOUT_3D:
    ...
    def plot_3d_network(self, paths, position=None, layout=None):
        ...
        # Init Graph Objects
        digraph = self.init_digraph(paths)

        # Positioning
        start = datetime.now()
        position = nx.random_layout(digraph, dim=3) if position is None else position
        position = KatalonGraph.update_position_layout(layout, digraph, position) if layout is not None else position
        ...
        node_traces = KatalonGraph.init_node_traces_3d(digraph, position)
        go_edges = None
        if edge_property is not None and edge_property.get('plot') is True:
            go_edges = KatalonGraph.init_edge_traces_3d(digraph, position)

        # Layout
        figure = KatalonGraph.layout_figure(node_traces, go_edges)
    ...
    def plot_2d_network(self, paths, position=None, **kwargs):
        ...
        node_traces = KatalonGraph.init_node_traces_2d(digraph, position)
        go_edges = None
        if edge_property is not None and edge_property.get('plot') is True:
            go_edges = KatalonGraph.init_edge_traces_2d(digraph, position)
    ...
    def get_layout_position_3d(self, paths, layout=VOLCANO_LAYOUT_3D):
        digraph = self.init_digraph(paths)
        position = nx.random_layout(digraph, dim=3)
        position = KatalonGraph.update_position_layout(layout, digraph, position)
        return digraph, position
    ...
    def highlight_path_2d(self, sel_path, paths, position):
        ...
        node_traces = KatalonGraph.init_node_traces_2d(digraph, position, colorscale='Greys')
        edge_x = []
        edge_y = []
        for edge in digraph.edges():
            x0, y0 = position[edge[0]]
        ...
        for sel_node in sel_path:
            ...
            sel_node_size.append(min([digraph.nodes[sel_node]['frequency'], KatalonGraph.NODE_MAX_SIZE]))
            sel_node_color.append(digraph.degree[sel_node])
            sel_node_text.append(KatalonGraph.get_text_node_info(digraph, sel_node))
    ...
    def highlight_path_3d(self, sel_path, paths, position):
        ...
        position = nx.random_layout(digraph, dim=3) if position is None else position
        # Construct Node traces and Edge traces of all graph
        node_traces = KatalonGraph.init_node_traces_3d(digraph, position, colorscale='Greys')
        edge_x = []
        edge_y = []
        edge_z = []
        for edge in digraph.edges():
        ...
        sel_node_trace = go.Scatter3d(x=sel_node_x, y=sel_node_y, z=sel_node_z, name='Sel Nodes',
                                      ...
                                      marker=dict(showscale=True, colorscale='greens',
                                                  ...
                                                  size=KatalonGraph.rescale_node_size(sel_node_size),
                                                  colorbar=dict(thickness=15, title='Node Connections',
                                                                xanchor='left', titleside='right'), line_width=2))
        sel_edge_x = []
        sel_edge_y = []
    ...
    def highlight_multi_path_2d(self, sel_paths, paths, position):
        ...
        for idx, sel_path in enumerate(sel_paths):
            ...
            sel_node_trace = go.Scatter(x=sel_node_x, y=sel_node_y, name='Sel Nodes {}'.format(idx),
                                        mode='markers', hoverinfo='text', text=sel_node_text,
                                        marker=dict(showscale=True, colorscale=KatalonGraph.COLOR_SCALE[idx],
                                                    reversescale=True, color=sel_node_color,
                                                    colorbar=dict(thickness=15, title='Node Connections',
                                                                  xanchor='left', titleside='right'), line_width=2))
            sel_edge_x = []
            ...
            sel_edge_trace = go.Scatter(x=sel_edge_x, y=sel_edge_y, name='Sel Edges {}'.format(idx),
                                        line=dict(width=5, color=KatalonGraph.COLOR[idx]), mode='lines')
    ...
    def highlight_cluster_3d(self, paths, position):
        ...
        position = nx.random_layout(digraph, dim=3) if position is None else position
        # Construct Node traces and Edge traces of all graph
        node_traces = KatalonGraph.init_node_traces_3d_with_condition(condition, digraph, position, colorscale='Greys')
        edge_traces = KatalonGraph.init_edge_traces_3d_with_condition(condition, digraph, position)
        # Layout all traces
